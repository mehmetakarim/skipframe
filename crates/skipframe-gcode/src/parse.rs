...
struct Parser {
    ...
    // --- completeness ----------------------------------------------------------------------
    /// Lines read so far; the last one is where a truncated file stops.
    lines: u64,
    /// `; total layer number: N` — the slicer's own count, written in the header.
    declared_layers: Option<u32>,
    /// `; EXECUTABLE_BLOCK_END` — written after the last printed move.
    saw_body_end: bool,
}
...
impl Parser {
    ...
    fn handle_comment(&mut self, raw: &[u8]) {
        ...
        // Read whatever the dialect: only acted on for the slicers known to write both.
        if c == "EXECUTABLE_BLOCK_END" {
            self.saw_body_end = true;
            return;
        }
        if let Some(n) = c.strip_prefix("total layer number:") {
            if let Ok(n) = n.trim().parse() {
                self.declared_layers = Some(n);
            }
            return;
        }
    ...
    /// Whether the file stops before the print its header describes.
    ///
    /// Bambu Studio and OrcaSlicer write the layer total at the top and `EXECUTABLE_BLOCK_END`
    /// after the last move — checked in real files from both. A file is called truncated only when
    /// the two agree: the end marker is missing *and* fewer layers were read than the header
    /// promised. Either on its own has innocent explanations (a hand-edited end block, a layer
    /// count that differs by how the start block is counted), and refusing a good file is worse
    /// than rendering one that is a layer short.
    ///
    /// Other slicers are not checked. There is no real file from them here to learn what they
    /// always write at the end, and guessing would mean refusing files that are fine.
    fn truncation(&self) -> Option<Error> {
        if !matches!(self.dialect, Dialect::OrcaSlicer | Dialect::BambuStudio) || self.saw_body_end
        {
            return None;
        }
        let declared = self.declared_layers?;
        let read = if self.ir.segment_count() == 0 {
            0
        } else {
            self.ir.layer_start.len() as u32
        };
        (read < declared).then_some(Error::Truncated {
            line: self.lines,
            layers_read: read,
...
mod tests {
    ...
    /// An Orca-style file: banner, declared layer total, `layers` layers of moves, and the end
    /// marker if `ended`.
    fn orca(declared: u32, layers: u32, ended: bool) -> String {
        let mut s = format!(
            "; generated by OrcaSlicer 2.4.2
; total layer number: {declared}
G90
M83
"
        );
        for l in 0..layers {
            let z = 0.2 * (l + 1) as f32;
            s += &format!(
                ";LAYER_CHANGE
;Z:{z:.1}
G1 Z{z:.1}
...
";
        ...
        s += "; filament used [g] = 1.0
";
        s
    }

    fn try_parse(src: &str) -> Result<Ir> {
        parse_stream(
    ...
    #[test]
    fn a_file_cut_before_its_last_layers_is_truncated() {
        let src = orca(3, 2, false);
        ...
        match try_parse(&src) {
            Err(Error::Truncated {
                ...
                assert_eq!((layers_read, layers_declared), (2, 3));
                assert_eq!(line, lines);
            }
            other => panic!(
                "expected truncated, got {:?}",
                other.map(|ir| ir.meta.layer_count)
            ),
        }
    }

    #[test]
    fn a_file_cut_inside_its_header_is_truncated_not_empty() {
...
";
        assert!(matches!(
            try_parse(src),
            Err(Error::Truncated { layers_read: 0, .. })
        ));
    }

    #[test]
    fn a_complete_file_is_not_truncated() {
        assert_eq!(try_parse(&orca(3, 3, true)).unwrap().meta.layer_count, 3);
    }

    /// Both signals must agree. Each alone has innocent explanations, and refusing a good file is
    /// the worse mistake.
    #[test]
    fn one_signal_alone_is_not_enough() {
        // All layers there, only the end marker missing — an edited or oddly exported end block.
        assert!(try_parse(&orca(3, 3, false)).is_ok());
        // End marker there, fewer layers than declared — the header disagrees with the body.
        assert!(try_parse(&orca(3, 2, true)).is_ok());
    }

    #[test]
    fn slicers_without_a_known_end_marker_are_never_called_truncated() {
        let prusa = orca(3, 2, false).replace("OrcaSlicer", "PrusaSlicer");
        assert!(try_parse(&prusa).is_ok());
    }
